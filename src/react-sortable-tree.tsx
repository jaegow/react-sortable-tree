...
class ReactSortableTree extends Component<ReactSortableTreeProps, ReactSortableTreeState> {
  ...
  render() {
    ...
    if (rows.length < 1) {
        ...
        <AutoSizer>
          {({ height, width }) => (
            <ScrollZoneVirtualList
              ...
              rowRenderer={({ index, style: rowStyle }: any) =>
      ...
      list = rows.map((row: any, index: any) =>
        this.renderRow(row, {
          listIndex: index,
          style: {
            height:
...
const SortableTreeWithoutDndContext = (props: any) => (
  <DndContext.Consumer>
    {({ dragDropManager }) =>
      dragDropManager === undefined ? null : <ReactSortableTree {...props} dragDropManager={dragDropManager} />
    }
  </DndContext.Consumer>
);
//
