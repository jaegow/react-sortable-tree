...
export function getVisibleNodeCount({ treeData }: any) {
  const traverse = (node: any) => {
    ...
    return 1 + node.children.reduce((total: any, currentNode: any) => total + traverse(currentNode), 0);
  };
...
function addNodeAtDepthAndIndex({
  ...
  const selfPath = (n: any) => (isPseudoRoot ? [] : [...path, getNodeKey({ node: n, treeIndex: currentIndex })]);
